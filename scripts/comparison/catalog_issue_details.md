...
- Generated: 2026-09-08T02:28:16
- Catalog: `scripts/comparison/known_issues.json`
- Catalog size: 56 issues (28 pending, 28 resolved)
- Issues rendered: 28
...
## B-01 — QI-Core baseline `qicore-2025-actual-results.csv` (post 2026-09-05 fresh re-run) tracks 1456 qic…

- **ID**: B-01
- **Title**: QI-Core baseline `qicore-2025-actual-results.csv` (post 2026-09-05 fresh re-run) tracks 1456 qicore-only cells across 26 measures where the fresh QI-Core engine output disagrees with the fixture MeasureReports' expectations (CMS engine matches; baseline is fresh, not stale, but cells remain)
- **Category**: baseline
...
- **Status**: **Confirmed** (refreshed 2026-09-05). 1456 qicore-only cells
  across 26 measures tracked. Pre-resolution catalog entry (B-01 v1, 2026-09-05
  03:54) reported 800 cells; the re-run exposed 877 new cells and resolved
  191, netting to 1456.
...
## C-02 — CMS157 — Cancer diagnosis not matching the "Cancer" valueset — RESOLVED via systemic repair of t…
...
- **Category**: content
- **Status**: Fixed — verified 63/0 on CMS engine (2026-09-07 21:14); QICore-side repair applied, engine re-run pending
- **Resolution**: pending
- **Affected measures**: CMS157
...
- **QICore side**: the 42 repaired files were mirrored into `dqm-content-qicore-2025`
  (verified byte-identical source; the 42 repaired files were the only files differing
  between the repos). The QICore baseline (`qicore-2025-actual-results.csv`, 2026-09-05)
  and its `TestCaseResult-*.json` set predate the repair, so the report's QICore column
  (760 failing / 1730 qicore-only cells) is stale until the QI-Core engine is re-run in
  that repo and the baseline regenerated via `extract_population_qicore.py --input ... --force`.
- **Category**: terminology / stale-content (confirmed), not an engine issue.

### Workaround

Run scripts/comparison/repair_valueset_cache.py to copy complete expansions from input-cache/txcache into input/vocabulary/valueset/external; CMS engine re-run confirms resolution; mirror the 42 repaired files into dqm-content-qicore-2025 and re-run the QI-Core engine + regen baseline
...
## C-05 — CMS157 Pain Intensity Quantified — RESOLVED on CMS engine: prior 'fixture MR/Encounter-type auth…
...
- **Category**: content
- **Status**: Fixed — verified on CMS engine (63/0, 2026-09-07 21:14); QICore-side re-run pending
- **Resolution**: pending
- **Affected measures**: CMS157FHIRPainIntensityQuantified

### Affected test cases (first 5 of 19)

- `CMS157FHIRPainIntensityQuantified` / `055640ae-dc71-4e1d-918b-e367013de209`
- `CMS157FHIRPainIntensityQuantified` / `233d84af-d725-4682-8253-d6c4e02da0d5`
- `CMS157FHIRPainIntensityQuantified` / `2c3f5ac5-6b7f-4bb8-a4fe-8faf0553b21e`
- `CMS157FHIRPainIntensityQuantified` / `51d8547c-f07f-4441-b616-f458f38e4506`
- `CMS157FHIRPainIntensityQuantified` / `5cca62ff-f856-4b8f-9902-6a018a4599cb`

_Full enumeration (19 case(s)): `scripts/comparison/known_issues.json` under the `affected_test_cases` field of this issue._
...
- **Residual**: the sequential `discrepancy_report.md` rows for CMS157 (CMS actual=expected vs
  QI-Core=0) and the 46 `qicore-only` cells are **stale-baseline artifacts**: `qicore-2025-actual-results.csv`
  (`2026-09-05`) and the QI-Core `TestCaseResult-*.json` set predate the valueset repair, and the
  QI-Core repo carried the identical 45 truncated files. Re-run the QI-Core engine in
  `dqm-content-qicore-2025` and regenerate the baseline to confirm the QICore column flip.
...
None needed — valuesets repaired via scripts/comparison/repair_valueset_cache.py; re-run the QI-Core engine (cql.explorer.project.execute-all in the QICore repo) and regenerate qicore-2025-actual-results.csv to close the residual classification
...
- **ID**: C-14
- **Title**: CMS72 / CMS104 / CMS646 / CMS71 residual fixture MR authoring gaps where both engines agree (Denominator-Exception / Denominator / Numerator / IP cells hand-authored in fixture MR but uncomputable from present resources)
- **Category**: content
- **Status**: **Confirmed**
- **Resolution**: pending
...
- **Cited in discrepancy_report.md**: 28
...
- **Status**: **Confirmed**
- **Resolution**: pending
- **Affected measures**: CMS1173, CMS871, CMS645, CMS646, CMS156
...
- **Status**: **Confirmed**
- **Resolution**: pending
- **Affected measures**: CMS1173, CMS156
...
Single `FHIR.Condition` retrieve replacing the union (62 site-level edits; 26 measures applied of 30; 4 pending Stage 3); inline `is`/`as` interim superseded; CMS133, CMS128 & CMS56 VERIFIED 2026-08-30; CMS131 applied 2026-08-30, verified 2026-08-31 (0007 report)
...
- **Cited in discrepancy_report.md**: 46

### Affected test cases (first 5 of 48)
...
_Full enumeration (48 case(s)): `scripts/comparison/known_issues.json` under the `affected_test_cases` field of this issue._
...
- **Cited in discrepancy_report.md**: 69
