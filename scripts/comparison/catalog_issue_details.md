...
- **Corroboration - CMS108/CMS190 VTE Prophylaxis (2026-09-08)**: 8 CMS108 test cases
  carry a `MedicationRequest`-with-`TaskRejected` rejection arm (`No VTE Prophylaxis Medication
  Administered Or Ordered`, lines 343-358) that evaluates empty despite fully correct data — same
  signature as CMS104 `5adc911a`. Cases: `182103c1`, `2eff6dbd`, `3c854f27`, `525e73f2`,
  `5f739500`, `91ff5f1a`, `d205878e`, `ff814452` (all `Numerator 1->0`). Verified data-side:
  MedicationRequest profile/status/intent/doNotPerform correct, medication code (e.g. RxNorm
  `1232086` for Rivaroxaban) present in the relevant VTE-prophylaxis valueset
  ...
  `T.code ~ "Fulfill"` holds. The retrieve + join still returns `[]`.
  **Previously mis-labeled E-17**; reclassified to E-12 2026-09-08.
  CMS190 uses the same `[TaskRejected]` join (lines 323, 355) — sweep pending.
...
- **Reclassified 2026-09-08**: the 8 CMS108 `MedicationRequest`-with-`TaskRejected`
  rejection cases (`182103c1`, `2eff6dbd`, `3c854f27`, `525e73f2`, `5f739500`, `91ff5f1a`,
  `d205878e`, `ff814452`) were mis-attributed to E-17; the true mechanism is the E-12
  `TaskRejected`-join-empty defect. Removed from E-17's affected-test-cases; tracked on E-12.
